...
_.path.replace_front = function(path, part, replacement){
...
_.path.shorten_tilde = function(path){
   if(path.startsWith(_.os.homedir())){
      let regex = new RegExp('^' + _.os.homedir());
...
_.fs = fs;
_.fs.insert_lines_after = function(path, target, new_lines){
   if(!Array.isArray(new_lines)){ new_lines = [new_lines]; }
   ...
   let target_index = lines.findIndex(function(line){ return line.includes(target) });

   if(target_index === -1){ target_index = (lines.length-2); }

   lines.splice(target_index + 1, 0, ...new_lines);
